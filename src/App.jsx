...
const apiBackend = {
  analyzeUrl: async (targetUrl, cookie = '') => {
    try {
      ...
      const response = await fetch(`/api/analyze?url=${encodeURIComponent(targetUrl)}`, { method: 'GET', headers: headers });
      if (!response.ok) {
...
export default function App() {
  ...
  const handleStartAnalysis = async () => {
    ...
    try {
      ...
      const items = (result.results || []).map((item, index) => ({
        ...
        downloadUrl: item.url, // Link này sẽ được api/download xử lý
        url: item.url, 
  ...
  const startMockAnalysis = () => {
    ...
    analysisIntervalRef.current = setInterval(() => {
      ...
      if (itemsFound >= 20) { clearInterval(analysisIntervalRef.current); setIsAnalyzing(false); }
    }, 300);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900/40 to-slate-900 text-slate-100 font-sans selection:bg-pink-500/30 pb-20">
        ...
        <div className="max-w-4xl mx-auto px-4 h-20 flex items-center justify-between">
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <div className="bg-gradient-to-tr from-indigo-500 to-pink-500 p-2 rounded-xl shadow-lg shadow-purple-500/20"><Sparkles size={20} className="text-white fill-white/20" /></div>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        {activeTab === 'download' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="bg-white/5 backdrop-blur-xl rounded-2xl p-6 shadow-2xl border border-white/10 space-y-6 relative overflow-hidden group">
              {/* COOKIE INPUT */}
              <div className="mb-4">
                 <button onClick={() => setShowAdvanced(!showAdvanced)} className="flex items-center gap-2 text-xs text-purple-300 hover:text-purple-200 transition-colors"><Settings size={14} /> Cấu hình nâng cao (Cookie Facebook)</button>
                 {showAdvanced && (
                    <div className="mt-3 p-4 bg-black/40 rounded-xl border border-purple-500/20 animate-in slide-in-from-top-2">
                       <label className="block text-xs text-slate-400 mb-2 flex items-center gap-1"><Key size={12} /> Cookie (c_user, xs...) - Dùng để tải video Private/Reels</label>
                  ...
                  <div className="p-4 bg-black/20 min-h-[300px] max-h-[500px] overflow-y-auto custom-scrollbar">
                     <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                        {displayMedia.map((item) => (
                           <MediaCard key={item.id} item={item} isAnalyzing={isAnalyzing} downloadState={downloadState} toggleSelection={toggleSelection} />
                        ))}
                      </div>
                  </div>
                  
                  {/* FOOTER: PATH & DOWNLOAD BUTTON */}
        ...
        {/* History Tab */}
        {activeTab === 'history' && <div className="text-center text-slate-500">History UI (Đang cập nhật)</div>}
      </main>
    </div>
  );
}